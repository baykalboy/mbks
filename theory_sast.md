...
SonarQube Community Edition 26.4.0 запускается как Java-сервис (порт 9000). Анализ проводится через `dotnet-sonarscanner begin / MSBuild / end` — с полноценной компиляцией через Roslyn.

**Сложность сборки:** DNN 9.1.0 использует устаревший формат `packages.config` со старым `NuGet.exe` v2.x, который не работает с nuget.org v3. Для успешного анализа потребовалось:
1. Заменить `NuGet.exe` на v7.x и создать `NuGet.Config` с явным указанием nuget.org v3
2. Собрать зависимости вручную: `ClientDependency.Core` → `DotNetNuke.Syndication` → `DotNetNuke.Library`

В результате `DotNetNuke.Library.csproj` (ключевой файл с уязвимым кодом) успешно скомпилирован и проанализирован Roslyn.

### 10.2 Метрики проекта после полного анализа
...
| Code Smells | 3 927 |
| Vulnerabilities | 4 |
| Security Hotspots | 48 |

### 10.3 Встроенные правила — результаты

| Тип | Серьёзность | Правило | Файл | Строка | Описание |
|---|---|---|---|---|---|
| VULNERABILITY | BLOCKER | `secrets:S6703` | `AdoNetAppender.cs` | 87 | Hardcoded credentials в конфигурации логгера log4net |
| SECURITY HOTSPOT | MEDIUM | `javascript:S5852` | различные `.js` | — | Потенциально уязвимые RegExp (48 срабатываний в JS) |

**Вывод:** несмотря на успешную компиляцию Library и обработку Roslyn, встроенные C# правила безопасности (S3649 — SQL Injection, S5773 — небезопасная десериализация) не нашли CVE-2017-9822. Паттерн `new XmlSerializer(Type.GetType(string))` — нетипичная конструкция, не покрытая стандартными шаблонами SonarQube. Это подтверждает необходимость кастомных правил для специфических CVE.
